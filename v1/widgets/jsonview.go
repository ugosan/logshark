...
func (self *JSONView) Draw(buf *Buffer) {
	self.Block.Draw(buf)
	...
	for row := self.topRow; row < len(self.Rows) && point.Y < self.Inner.Max.Y; row++ {
		cells := parser.ParseStyles(self.Rows[row], self.TextStyle)
		if self.WrapText {
...
func (self *JSONView) ScrollAmount(amount int) {
...
func (self *JSONView) ScrollUp() {
	self.ScrollAmount(-1)
}

func (self *JSONView) ScrollDown() {
	self.ScrollAmount(1)
}

func (self *JSONView) ScrollPageUp() {
...
func (self *JSONView) ScrollPageDown() {
	self.ScrollAmount(self.Inner.Dy())
}

func (self *JSONView) ScrollHalfPageUp() {
	self.ScrollAmount(-int(FloorFloat64(float64(self.Inner.Dy()) / 2)))
}

func (self *JSONView) ScrollHalfPageDown() {
	self.ScrollAmount(int(FloorFloat64(float64(self.Inner.Dy()) / 2)))
}

func (self *JSONView) ScrollTop() {
	self.SelectedRow = 0
}

func (self *JSONView) ScrollBottom() {
	self.SelectedRow = len(self.Rows) - 1
}
